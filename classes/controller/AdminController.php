...
{
    //Fonction à activer si backoffice lent
    ...
    public function __construct($forceControllerName = '', $default_theme_name = 'default')
    {
        ...
        if (defined('_PS_BO_ALL_THEMES_DIR_')) {
            if (
                defined('_PS_BO_DEFAULT_THEME_') && _PS_BO_DEFAULT_THEME_
    ...
    public function processStatus()
    {
        if (Validate::isLoadedObject($object = $this->loadObject())) {
            if ($object->toggleStatus()) {
                $matches = [];
                if (
                    preg_match('/[\?|&]controller=([^&]*)/', (string) $_SERVER['HTTP_REFERER'], $matches) !== false
    ...
    public function initProcess()
    {
        ...
        if (
            Tools::isSubmit('submitFilter' . $this->list_id)
