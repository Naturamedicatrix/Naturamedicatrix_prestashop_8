...
class WkProductSubscriptionCronModuleFrontController extends ModuleFrontController
{
    public $todayDate;
    ...
    public function init()
    {
        ...
        if (isset($token)
            && Configuration::get('WK_SUBSCRIPTION_CRON_TOKEN') == $token
        ) {
            ...
            if ($tomScheduledOrders) {
                foreach ($tomScheduledOrders as $subsData) {
                    if ($subsData['payment_module'] == 'wkstripepayment'
                        ...
                        if (
                            Configuration::hasKey('WK_STRIPE_CREATE_ORDER_ON_SUB_RENEW')
                            && Configuration::get('WK_STRIPE_CREATE_ORDER_ON_SUB_RENEW')
                        ) {
                            continue;
                        }
                        ...
                        if (
                            Configuration::hasKey('WK_PPS_CREATE_RENEWAL_ORDER')
                            && Configuration::get('WK_PPS_CREATE_RENEWAL_ORDER')
                        ) {
                            continue;
                        }
                ...
                foreach ($todayScheduledOrders as $subscriptionData) {
                    ...
                    if ($idCart) {
                        ...
                        if (Validate::isLoadedObject($objOrder)) {
                            ...
                            if ($cartRules) {
                                $firstTempCart = WkSubscriptionCartProducts::getByIdCart($objOrder->id_cart, true);
                                foreach ($cartRules as $cartRule) {
                                    $objCartRule = new CartRule($cartRule['id_cart_rule']);
                                    if (Validate::isLoadedObject($objCartRule)) {
                                        $cartRuleName = [];
                                        if ($firstTempCart[0]['id_cart_rule'] == $cartRule['id_cart_rule']) {
                                            ...
                                            foreach (Language::getLanguages(false) as $lang) {
                                                $cartRuleName[$lang['id_lang']] = $objCartRule->name[$lang['id_lang']];
                                            }
                                        }
                                        ...
                                        if ($objCartRule->reduction_percent > 0) {
                                            $objCartRuleNew->reduction_percent = $objCartRule->reduction_percent;
                                        } elseif ($objCartRule->reduction_amount > 0) {
                                            $objCartRuleNew->reduction_amount = $objCartRule->reduction_amount;
                                        }

                                        if ($objCartRule->free_shipping) {
                                            $objCartRuleNew->free_shipping = true;
                                        }

                                        if ($objCartRule->gift_product) {
                                            $objCartRuleNew->gift_product = $objCartRule->gift_product;
                                            $objCartRuleNew->gift_product_attribute = $objCartRule->gift_product_attribute;
                                        }
                                        ...
                                        $objCartRuleNew->reduction_tax = $objCartRule->reduction_tax;
                                        $objCartRuleNew->quantity_per_user = 1;
                                        $objCartRuleNew->priority = 1;
                                        $objCartRuleNew->partial_use = false;
                                        $objCartRuleNew->code = '';
                                        $objCartRuleNew->minimum_amount = 0;
                                        $objCartRuleNew->product_restriction = true;
                                        $objCartRuleNew->shop_restriction = Shop::isFeatureActive() ? true : false;
                                        $objCartRuleNew->reduction_product = (int) $subscriptionData['id_product'];
                                        $objCartRuleNew->active = true;
                                        $objCartRuleNew->save();
            ...
            // Get resume subscriptions
            $resumeSubscriptions = $this->getTodayResumeSubscriptions();
            if ($resumeSubscriptions) {
                foreach ($resumeSubscriptions as $subscriptionData) {
                    $data = $objGlobal->resumeSupscription($subscriptionData['id_wk_subscription_subscriber_products']);
                    if ($data['status']) {
                        ++$total_subs_resume;
                    }
                }
    ...
    /**
     * Get resume subscription for today
     *
     * @return array
     */
    private function getTodayResumeSubscriptions()
    {
        $objGlobal = new WkProductSubscriptionGlobal();

        return $objGlobal->getTodayResumeSubscriptions($this->todayDate);
    }
        ...
        if ($subscriptionData) {
            ...
            if ($subscriptionData['id_customization']) {
                ...
                if (Validate::isLoadedObject($objCustomization)) {
                    ...
                    if ($newCustObj && Validate::isLoadedObject($newCustObj)) {
    ...
    private function createSubscriptionOrder($subscriptionData, $id_cart)
    {
        ...
        if (Validate::isModuleName($module_name)) {
            ...
            // check if wallet amount less than cart total amount then skip order creation
            if ($module_name == 'wkcustomerwallet') {
                $walletAmount = CustomerWallet::getWalletTotalAmount($subscriptionData['id_customer'], $cart->id_currency);
                if ($walletAmount < $cart->getOrderTotal(true, Cart::BOTH)) {
                    return false;
                }
            }
