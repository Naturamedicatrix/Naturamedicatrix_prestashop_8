    ...
    public function validateOrder(
        ...
        if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false) {
            ...
            if (Module::isEnabled('wkproductsubscription')) {
                ...
                $whProductWise = $objSubsCart->getSplittedProductList($this->context->cart);
                $orderPackage = [];
                foreach ($package_list as $id_address => $packages) {
                    foreach ($packages as $id_package => $package) {
                        foreach ($package['product_list'] as $product) {
                            $id_carrier = 0;
                            $idWarehouse = $whProductWise[$product['id_product']];
                            $idWarehouse = $idWarehouse ? $idWarehouse : 0;
                            $packageKey = $id_carrier . '-' . $idWarehouse;
                ...
                if (isset($order->id)) {
                    ...
                    foreach ($order->product_list as $product) {
                        ...
                        if (version_compare(_PS_VERSION_, '1.7.6.9', '>')) {
                            ...
                            $product_price = Product::getTaxCalculationMethod() == PS_TAX_EXC ? Tools::ps_round($price, Configuration::get('PS_PRICE_DISPLAY_PRECISION')) : $price_wt;
                        }
                    ...
                    if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id) {
                        ...
                        $data = [
                            ...
                            '{id_order}' => $order->id,
                            '{date}' => Tools::displayDate(date('Y-m-d H:i:s'), null),
