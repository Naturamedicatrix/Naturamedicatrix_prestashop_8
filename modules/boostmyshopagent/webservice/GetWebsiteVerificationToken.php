...
$data = ['success' => true, 'message' => '', 'token' => ''];
...
try {
    ...
    $token = Configuration::get('BMS_GSC_AGENT_SITE_VERIFICATION_TOKEN');
    if (empty($token)) {
        http_response_code(403);
        throw new Exception('Website verification token not configured');
    }
