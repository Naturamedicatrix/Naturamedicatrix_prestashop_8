...
$data = ['success' => true, 'message' => '', 'id' => '', 'label' => ''];
...
try {
    ...
    $id = Configuration::get('BMS_GSC_CONVERSION_TRACKING_ID');
    $label = Configuration::get('BMS_GSC_CONVERSION_TRACKING_LABEL');
    $gad_id = Configuration::get('BMS_GSC_GAD_ID');

    if (empty($id) || empty($label) || empty($gad_id)) {
        http_response_code(403);
        throw new Exception('Conversion detail not configured');
    }
