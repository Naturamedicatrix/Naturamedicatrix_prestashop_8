...
class IqitMegaMenu extends Module implements WidgetInterface
{
    ...
    public function __construct()
    {
        ...
        $this->defaults_mobile = [
            'mobile_menu' => 'HOME0,CAT3,CAT26',
            'mobile_menu_content' => 'from-horizontal',
            'mobile_menu_depth' => 3,
            'hor_mb_bg' => '#000000',
            'hor_mb_txt' => '#ffffff',
...
');
    ...
    public function getContent()
    {
        ...
        Media::addJsDef(['iqitsearch_url' => $this->context->link->getAdminLink('AdminModules', true) . '&configure=' . $this->name]);
        ...
        if (Tools::isSubmit('addTab') || (Tools::isSubmit('id_tab') && !Tools::isSubmit('submitAddTab') && IqitMenuTab::tabExists((int) Tools::getValue('id_tab')))) {
            ...
            file_put_contents($this->getLocalPath() . 'export/' . $file_name, json_encode($var), true);
            fclose($fd);
            Tools::redirect(_PS_BASE_URL_ . __PS_BASE_URI__ . 'modules/' . $this->name . '/export/' . $file_name);
        } elseif (Tools::isSubmit('importConfiguration')) {
            if (isset($_FILES['uploadConfig'], $_FILES['uploadConfig']['tmp_name']) && $_FILES['uploadConfig']['tmp_name']) {
            ...
            $fd = fopen($this->getLocalPath() . $file_name, 'w+');
            file_put_contents($this->getLocalPath() . 'export/' . $file_name, json_encode($var));
            fclose($fd);
            Tools::redirect(_PS_BASE_URL_ . __PS_BASE_URI__ . 'modules/' . $this->name . '/export/' . $file_name);
        } elseif (Tools::isSubmit('importTabs')) {
            if (isset($_FILES['uploadTabs'], $_FILES['uploadTabs']['tmp_name'])  && $_FILES['uploadTabs']['tmp_name']) {
                $str = Tools::file_get_contents($_FILES['uploadTabs']['tmp_name']);
                $arr = json_decode($str, true);

                foreach ($arr as $id_tab => $tab) {
                    $newTab = new IqitMenuTab();

                    foreach ($tab as $key => $val) {
                        if (property_exists('IqitMenuTab', $key)) {
                            $newTab->$key = $val;
                        }
                    }

                    $newTab->id_shop = (int) Context::getContext()->shop->id;
                    $newTab->id_shop_list = [(int) Context::getContext()->shop->id];
            ...
            foreach ($shops as $shop_id) {
                ...
                Configuration::updateValue($this->config_name . '_mobile_menu', str_replace(['LNK' . $id_iqitmenulinks . ',', 'LNK' . $id_iqitmenulinks], '', Configuration::get($this->config_name . '_mobile_menu')));
        ...
        $this->_html .= $this->renderTabsLinks(1);
        $this->_html .= $this->renderTabsLinks(2);
        $this->_html .= $this->renderTabsLinks(3);
        // $this->_html .= $this->renderTabsLinks(4);
        $this->_html .= $this->renderHtmlContents();
    ...
    private function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitAddTab')) {
            ...
            $tab->group_access = json_encode($group_access);
    ...
    public function renderAddForm()
    {
        ...
        $fields_form['form']['input'][] = ['type' => 'hidden', 'name' => 'menu_type'];

        $selected_tabs = '';
        $submenu_content = '';
        $submenu_content_format = [];
        if (Tools::isSubmit('id_tab') && IqitMenuTab::tabExists((int) Tools::getValue('id_tab'))) {
            $tab = new IqitMenuTab((int) Tools::getValue('id_tab'));
            $fields_form['form']['input'][] = ['type' => 'hidden', 'name' => 'id_tab'];
    ...
    public function getAddFieldsValues()
    {
        ...
        if (Tools::isSubmit('id_tab') && IqitMenuTab::tabExists((int) Tools::getValue('id_tab'))) {
            ...
            $group_access = json_decode($tab->group_access);
    ...
    public function renderAddHtmlForm()
    {
        ...
        if (Tools::isSubmit('id_html') && IqitMenuHtml::htmlExists((int) Tools::getValue('id_html'))) {
            $fields_form['form']['input'][] = ['type' => 'hidden', 'name' => 'id_html'];
        }
        ...
        foreach ($dataset as $id => &$node) {
            ...
            if ($node['parentId'] === 0) {
                ...
                if (!isset($dataset[$node['parentId']]['children'])) {
                    $dataset[$node['parentId']]['children'] = [];
                }
    ...
    {
        usort($passedTree, [$this, 'sortByPosition']);
    ...
    protected function renderForm()
    {
            ...
            'form' => [
                ...
                'input' => [
                    [
                        ...
                        'desc' => $this->l('Affect only simple selector menu type'),
                        'options' => [
                            'query' => [
                                [
                                    'id_option' => 4,
                    ...
                    [
                        'row_title' => $this->l('Simple selector'),
                        'type' => 'link_choice',
                        'label' => '',
                        'name' => 'link',
                        'lang' => true,
                    ...
                    [
                        'row_title' => $this->l('Menu content'),
                        'upper_separator' => true,
                        'type' => 'color',
                        'label' => $this->l('Background color - first level'),
                        'name' => 'hor_mb_c_bg',
                        'size' => 30,
                    ],
                    [
                        'type' => 'color',
                        'label' => $this->l('Background color - second level'),
                        'name' => 'hor_mb_csl_bg',
                        'size' => 30,
                    ],
                    [
        ...
        if (Tools::getIsset('updateiqitmenulinks') && !Tools::getValue('updateiqitmenulinks')) {
            $fields_form_custom['form']['submit'] = [
        ...
        if (Tools::isSubmit('updateiqitmenulinks')) {
            $fields_form_custom['form']['input'][] = ['type' => 'hidden', 'name' => 'updatelink'];
            $fields_form_custom['form']['input'][] = ['type' => 'hidden', 'name' => 'id_iqitmenulinks'];
        }
        ...
        return $helper->generateForm([$fields_form_global, $fields_form_vertical, $fields_form_mobile, $fields_form_submenutabs, $fields_form_submenustyl, $fields_form_html, $fields_form_ietool, $fields_form_custom]);
    }
...
.cbp-hrmenu  .cbp-hrsub-inner, .cbp-hrmenu ul.cbp-hrsub-level2, .cbp-hrsub-haslevel3 > .cbp-category-link-w > ul {
    ...
    public function hookDisplayHeader()
    {
        ...
        $jsVars['mobileType'] = Configuration::get($this->config_name . '_mobile_menu_content');
    ...
    public function _prepareHook()
    {
        ...
        $mobile_menu_content = Configuration::get($this->config_name . '_mobile_menu_content');
    ...
    public function renderSelectedTabsSelect($tabs)
    {
        ...
        if (Tools::strlen($tabs)) {
            ...
            foreach ($tabs as $tab_id) {
                $predefiniedTabs[] = new IqitMenuTab($tab_id, $id_lang, $id_shop);
            }
        }

        $this->context->smarty->assign([
            'tabs' => $predefiniedTabs,
        ]);

        return $this->fetch('module:' . $this->name . '/views/templates/admin/selected_predefinied_tabs.tpl');
    }
    ...
    public function renderChoicesTabsSelect()
    {
        ...
        return $this->fetch('module:' . $this->name . '/views/templates/admin/available_predefinied_tabs.tpl');
    }
    ...
    public function renderChoicesSelect($single = false, $name = null, $class = null, $mobile = false)
    {
        ...
        $cmsPages = $this->getCMSOptions(0, 1, $this->context->language->id, $items, $single);
        ...
        foreach ($manufacturers as $manufacturer) {
            if (!in_array('MAN' . $manufacturer['id_manufacturer'], $items)) {
                $manufacturersOptions[] = [
                    'value' => 'MAN' . $manufacturer['id_manufacturer'],
                    'name' => $spacer . $manufacturer['name'],
                ];
            }
        }

        $categories = $this->generateCategoriesOption(Category::GetNestedCategories(null, (int) $this->context->language->id, true), $single);
        ...
        foreach ($links as $link) {
            if ($link['label'] == '') {
                ...
                if (!in_array('LNK' . (int) $link[0]['id_iqitmenulinks'], $items)) {
                    $linksOptions[] = [
                        'value' => 'LNK' . (int) $link[0]['id_iqitmenulinks'],
                        'name' => $spacer . Tools::safeOutput($link[0]['label']),
                    ];
                }
            } elseif (!in_array('LNK' . (int) $link['id_iqitmenulinks'], $items)) {
                $linksOptions[] = [
                    'value' => 'LNK' . (int) $link['id_iqitmenulinks'],
                    'name' => $spacer . Tools::safeOutput($link['label']),
                ];
            }
        }
        ...
        return $this->fetch('module:' . $this->name . '/views/templates/admin/choices_select.tpl');
    }
    ...
    private function makeMenuOptionMobile()
    {
        ...
        return $this->fetch('module:' . $this->name . '/views/templates/admin/selected_mobilemenu.tpl');
    }
    ...
    private function getCMSOptions($parent = 0, $depth = 1, $id_lang = false, $items_to_skip = null, $single = false, $id_shop = false)
    {
        ...
        foreach ($categories as $category) {
            if (isset($items_to_skip) && !in_array('CMS_CAT' . $category['id_cms_category'], $items_to_skip)) {
                $optionsArray[] = [
                    'value' => 'CMS_CAT' . $category['id_cms_category'],
                    'name' => $spacer . $category['name'],
                    'type' => 'category',
                    'pages' => $this->getCMSOptions($category['id_cms_category'], (int) $depth + 1, (int) $id_lang, $items_to_skip, $single),
                ];
            }
        }
        ...
        foreach ($pages as $page) {
            if (isset($items_to_skip) && !in_array('CMS' . $page['id_cms'], $items_to_skip)) {
                $optionsArray[] = [
                    'value' => 'CMS' . $page['id_cms'],
                    'name' => $spacer . $page['meta_title'],
                    'type' => 'page',
                ];
            }
        }
        ...
        return $this->fetch('module:' . $this->name . '/views/templates/admin/cms_option_select.tpl');
    }
    ...
    private function generateCategoriesOption($categories, $single = false)
    {
        ...
        foreach ($categories as $key => $category) {
            ...
            if (isset($category['children']) && !empty($category['children'])) {
                $subcats = $this->generateCategoriesOption($category['children'], $single);
            }

            $optionsArray[] = [
                'value' => 'CAT' . (int) $category['id_category'],
                'name' => str_repeat('-', $this->spacer_size * (int) $category['level_depth']) . $category['name'] . ' (' . $shop->name . ')',
        ...
        return $this->fetch('module:' . $this->name . '/views/templates/admin/categories_option_select.tpl');
    }
    ...
    protected function getCMSMenuItems($parent, $depth = 1, $id_lang = false)
    {
        ...
        if (count($categories) || count($pages)) {
            ...
            foreach ($pages as $page) {
                ...
                $links = $cms->getLinks((int) $id_lang, [(int) $cms->id]);
    ...
    private function generateCategoriesMenu($categories, $detph_limit, $current_depth = 1, $subcat_limit = null, $subcat_count = null, $subcats = false, $node = null)
    {
        ...
        foreach ($categories as $key => $category) {
            ...
            if (isset($subcat_limit, $subcat_count) && ($subcat_count >= $subcat_limit)) {
                return $return_categories;
            }
    ...
    private function installSamples()
    {
        ...
        $group_access = json_encode($group_access);
