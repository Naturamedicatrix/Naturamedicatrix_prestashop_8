...
class WkDonationInfo extends ObjectModel
{
    ...
    public static $definition = [
        ...
        'fields' => [
            ...
            'show_donate_button' => ['type' => self::TYPE_INT, 'validate' => 'isUnsignedInt', 'shop' => true],
            'adv_title_color' => ['type' => self::TYPE_STRING, 'validate' => 'isColor', 'shop' => true],
            'adv_desc_color' => ['type' => self::TYPE_STRING, 'validate' => 'isColor', 'shop' => true],
            'button_text_color' => ['type' => self::TYPE_STRING, 'validate' => 'isColor', 'shop' => true],
            'button_border_color' => ['type' => self::TYPE_STRING, 'validate' => 'isColor', 'shop' => true],
    ...
    {
        return Db::getInstance()->executeS(
            'SELECT DISTINCT `id_donation_info` FROM `' . _DB_PREFIX_ . 'wk_donation_info` di
            LEFT JOIN `' . _DB_PREFIX_ . 'stock_available` sa
            ON di.`id_product` = sa.`id_product`
            LEFT JOIN `' . _DB_PREFIX_ . 'product_shop` ps
            ON ps.`id_product` = di.`id_product`
            WHERE di.`show_at_checkout` = 1 AND di.`active` = 1  AND ps.`id_shop` = ' . (int) $idShop . '
    ...
    public static function getByCustomerDonationProduct()
    {
        $sql = 'SELECT di.`id_product` FROM `' . _DB_PREFIX_ . 'wk_donation_info` di WHERE di.`is_global` = 0 AND di.`active` = 1';
