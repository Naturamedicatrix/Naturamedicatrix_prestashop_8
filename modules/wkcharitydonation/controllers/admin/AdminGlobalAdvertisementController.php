...
class AdminGlobalAdvertisementController extends ModuleAdminController
{
    ...
    public function processSave()
    {
        ...
        $cleanedGlobalAdvDescription = strip_tags($description); // Remove HTML tags
        $cleanedGlobalAdvDescription = html_entity_decode($cleanedGlobalAdvDescription, ENT_QUOTES, 'UTF-8'); // Convert entities like &nbsp; to spaces
        $cleanedGlobalAdvDescription = trim($cleanedGlobalAdvDescription); // Remove leading/trailing spaces
        $cleanedGlobalAdvDescription = preg_replace('/[\p{Z}\s]+/u', '', $cleanedGlobalAdvDescription); // Remove all whitespace characters

        if (!$cleanedGlobalAdvDescription) {
                    ...
                    if (in_array(WkDonationDisplayPlaces::WK_DONATION_HOOK_LEFT, $donationhook)
                        ...
                        if ($imgLeftRight = $_FILES['background_image_left_right']) {
                            if (empty($imgLeftRight['name'])) {
                                ...
                                $this->errors[] = $this->customErrors(16);
                            }
                        }
                        break;
                    }
    ...
    public function customErrors($err)
    {
        $errorsArray = [
            ...
            14 => $this->l('Image format not recognized for header/footer advertisement, allowed formats are: .gif, .jpg, .png, .jpeg, .webp'),
            15 => $this->l('Background image is required for left/right advertisement'),
            16 => $this->l('Image format not recognized for left/right advertisement, allowed formats are: .gif, .jpg, .png, .jpeg .webp'),
