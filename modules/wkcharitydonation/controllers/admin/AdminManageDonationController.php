...
class AdminManageDonationController extends ModuleAdminController
{
    ...
    public function __construct()
    {
        ...
        $this->_join .= 'LEFT JOIN `' . _DB_PREFIX_ . 'wk_donation_info_shop` ds ON (ds.`id_donation_info` = a.`id_donation_info`)';
        $this->_where = ' AND dl.`id_lang` = ' . (int) $this->context->language->id . " AND a.`is_global` = '0' AND ds.`id_shop` = " . (int) Context::getContext()->shop->id;
        ...
        $this->fields_list = [
            ...
            'position' => [
                ...
                'hint' => $this->l('Priority define the ordering in which multiple donations will be shown'),
    ...
    public function renderForm()
    {
        ...
        $this->context->smarty->assign(
            [
                ...
                'active_tab' => Tools::getValue('active_tab') ? Tools::getValue('active_tab') : Tools::getValue('tab'),
            ]
        );
        $this->fields_form = [
            'submit' => [
    ...
    public function processSave()
    {
        ...
        $description = Tools::getValue('description_' . $defaultLangId);
        $cleanedDonationDescription = strip_tags($description);
        $cleanedDonationDescription = html_entity_decode($cleanedDonationDescription, ENT_QUOTES, 'UTF-8');
        $cleanedDonationDescription = trim($cleanedDonationDescription);
        $cleanedDonationDescription = preg_replace('/[\p{Z}\s]+/u', '', $cleanedDonationDescription);
        if (!$cleanedDonationDescription) {
            $this->errors[] = sprintf(
            ...
            $this->errors[] = $this->customErrors(19);
        } else {
            foreach ($languages as $language) {
        ...
        if ($advertise) {
            if (!trim(Tools::getValue('advertisement_title_' . $defaultLangId))) {
                $this->errors[] = $this->customErrors(1);
            } else {
                foreach ($languages as $language) {
                    if (!Validate::isCatalogName(Tools::getValue('advertisement_title_' . $language['id_lang']))) {
                        $this->errors[] = $this->customErrors(2);
                    }
                }
            }
            $description = Tools::getValue('advertisement_description_' . $defaultLangId);
            $cleanedAdvDescription = strip_tags($description);
            $cleanedAdvDescription = html_entity_decode($cleanedAdvDescription, ENT_QUOTES, 'UTF-8');
            $cleanedAdvDescription = trim($cleanedAdvDescription);
            $cleanedAdvDescription = preg_replace('/[\p{Z}\s]+/u', '', $cleanedAdvDescription);
            if (!$cleanedAdvDescription) {
                $this->errors[] = $this->customErrors(3);
            } elseif (preg_match('/<\s?[^\>]*\/?\s?>/i', html_entity_decode(strip_tags(Tools::getValue('advertisement_description_' . $defaultLangId))))) {
                $this->errors[] = $this->customErrors(18);
            } else {
                foreach ($languages as $language) {
                    if (!Validate::isCleanHtml(Tools::getValue('advertisement_description_' . $language['id_lang']))
                    ) {
                        $this->errors[] = $this->customErrors(4);
                    }
                }
            }
        }
        ...
        foreach ($languages as $language) {
            if (!Validate::isGenericName(Tools::getValue('donate_button_text_' . $language['id_lang']))) {
                $this->errors[] = $this->customErrors(5);
            }
        }
        ...
        if ($advertise) {
            if (empty($donationPageHook)) {
                ...
                if (!$imagePath_left_right) {
                            ...
                            if ($imgLeftRight = $_FILES['background_image_left_right']) {
                                if (empty($imgLeftRight['name'])) {
                                    if (!$this->getAdvertisementBannerImagePath(
                                        ...
                                        $this->errors[] = $this->customErrors(14);
                                    }
                                    ...
                                    $this->errors[] = $this->customErrors(15);
                                }
                            }
                            break;
                        }
                    ...
                    if ($imgLeftRight = $_FILES['background_image_left_right']) {
                        if (empty($imgLeftRight['name'])) {
                            if (isset($imgLeftRight['tmp_name']) && $imgLeftRight['tmp_name'] != '') {
    ...
    public function setMedia($isNewTheme = false)
    {
        ...
        $jsVars = [
            ...
            'confirm_title' => $this->l('Confirm'),
            'confirm_btn_text' => $this->l('Confirm'),
            'cancle_btn_text' => $this->l('Cancel'),
            'confirm_description' => $this->l('Are you sure you want to delete image.'),
        ];
        ...
        $this->addCSS(_MODULE_DIR_ . $this->module->name . '/views/css/lib/jquery-confirm/jquery-confirm.min.css');
        $this->addJS(_MODULE_DIR_ . $this->module->name . '/views/js/lib/jquery-confirm/jquery-confirm.min.js');
    ...
    public function customErrors($err)
    {
        $errorsArray = [
