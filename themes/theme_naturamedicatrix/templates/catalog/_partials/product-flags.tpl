 ...
 {block name='product_flags'}
    <ul class="product-flags js-product-flags mb-0">
        ...
        {foreach from=$product.flags item=flag}
            {if $flag.type == 'new'}
                <li class="mb-0 product-flag {$flag.type}">{$flag.label}</li>
                {break}
            {/if}
        {/foreach}
        ...
        {foreach from=$product.flags item=flag}
          {if $flag.type != 'out_of_stock' && $flag.type != 'new'}
            {if $flag.type == 'discount'}
                <li class="mb-0 product-flag {$flag.type}">{$flag.label} {if isset($product.dlu_checkbox) && $product.dlu_checkbox == 1}<span class="dlc-text">DLC courte</span>{/if}</li>
            {else}
                <li class="mb-0 product-flag {$flag.type}">{$flag.label}</li>
            {/if}
          {/if}
