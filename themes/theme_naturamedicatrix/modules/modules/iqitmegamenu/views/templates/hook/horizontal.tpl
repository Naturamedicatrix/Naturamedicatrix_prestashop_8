...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    const menu = document.querySelector('.cbp-hrmenu');

    const observer = new MutationObserver(() => {
      const submenuOpen = menu.querySelector('.cbp-hropen.cbp-has-submenu');
      document.body.classList.toggle('cbp-blur-active', !!submenuOpen);
    });
	...
	<div  class="iqitmegamenu-wrapper cbp-hor-width-{$menu_settings.hor_width} iqitmegamenu-all clearfix">
		<div id="iqitmegamenu-horizontal" class="iqitmegamenu  cbp-nosticky {if $menu_settings.hor_s_transparent && $menu_settings.hor_sticky} cbp-sticky-transparent{/if}" role="navigation">
			<div>
				...
				<nav id="cbp-hrmenu" class="cbp-hrmenu cbp-horizontal cbp-hrsub-narrow  {if $menu_settings.hor_animation==1}cbp-fade{/if} {if $menu_settings.hor_animation==2}cbp-fade-slide-bottom{/if} {if $menu_settings.hor_animation==3}cbp-fade-slide-top{/if} {if !$menu_settings.hor_arrow} cbp-submenu-notarrowed{/if} {if !$menu_settings.hor_arrow} cbp-submenu-notarrowed{/if} {if $menu_settings.hor_center} cbp-menu-centered{/if} ">
					<ul>
						{foreach $horizontal_menu as $tab}
						<li id="cbp-hrmenu-tab-{$tab.id_tab}" class="cbp-hrmenu-tab cbp-hrmenu-tab-{$tab.id_tab}{if $tab.active_label} cbp-onlyicon{/if}{if $tab.float} pull-right cbp-pulled-right{/if} {if $tab.submenu_type && !empty($tab.submenu_content)} cbp-has-submenu{/if}">
	{if $tab.url_type == 2}<a role="button" class="cbp-empty-mlink">{else}<a href="{$tab.url}" {if $tab.new_window}target="_blank"{/if}>{/if}
			...
			<div id="iqitmegamenu-mobile">
				...
				<div id="iqitmegamenu-shower" class="iqitmegamenu-shower clearfix pl-0 pr-1.5 py-1.5">
					<i class="bi bi-list text-3xl text-gray-900 leading-none"></i>
				</div>
				<div id="iqitmegamenu-mobile-content" >
				<div class="cbp-mobilesubmenu">
					<ul id="iqitmegamenu-accordion" class="cbp-spmenu cbp-spmenu-vertical cbp-spmenu-left fixed inset-0 w-full h-full bg-white z-50 overflow-y-auto">
						{include file="module:iqitmegamenu/views/templates/hook/_partials/mobile_menu.tpl" menu=$mobile_menu}
					</ul>
				</div>
  			<div id="cbp-spmenu-overlay" class="cbp-spmenu-overlay"><div id="cbp-close-mobile" class="close-btn-ui"><i class="bi bi-x-lg"></i></div></div>
  					</div>
  			</div>
